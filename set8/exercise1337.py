...
def fizz_buzz() -> List:
    ...
    fizz_buzz_list = []
    # your code here
    for i in range(1,101): # This range will give numbers 1 to 100 
...
def set_it_on_fire(input_string="very naughty boy") -> str:
    ...
    """
    fire = '🔥'
    return fire + fire.join(input_string.upper()) + fire
...
def best_letter_for_pets() -> str:
    ...
    for letter in the_alphabet: 
        x = len(pet_filter(letter)) #Length of list of pets whose name contains the character 'letter' 
        if x >= longest:
            longest = x 
